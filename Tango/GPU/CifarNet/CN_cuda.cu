...
void NeuralNetwork()
{
	...
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "1 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMalloc((void**) &Layer1_Weights_GPU, 2400* NUM * sizeof(double));
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "2 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMalloc((void**) &Data_Layer_GPU_R, 32*32* NUM * sizeof(int));
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "3 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMalloc((void**) &Data_Layer_GPU_G, 32*32* NUM * sizeof(int));
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "4 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	err = cudaMalloc((void**) &Data_Layer_GPU_B, 32*32* NUM * sizeof(int));
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "5 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	...
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "6 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	...
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "7 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	...
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "8 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	...
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "9 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	//Execute Third Layer
	...
	printf("num=%d\n",NUM);
	printf("size:%lld\n",64*4*4*64* NUM * sizeof(double)/1024/1024/1024);
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "10 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
	...
	if (err != cudaSuccess)
        {
        	fprintf(stderr, "11 Failed to allocate device data (error code %s)!\n", cudaGetErrorString(err));
        	exit(EXIT_FAILURE);
        }
