...
#define GPU_NAME "NVIDIA GeForce RTX 3060"
...
void monitor_power(nvmlDevice_t device)
{
    ...
    printf("%.3f,%d,%d,%d\n", (float)power/1000, temp, sm_clock, mem_clock);
    return;
...
void usage() {
    printf("Usage: binary [monitoring period (ms)] default 5ms\n");
}
...
{
    unsigned int sleep_useconds;
    if (argc < 2) {
        usage();
        sleep_useconds = 5000;
    }
    else{
	sleep_useconds = atoi(argv[1])*1000;
    }
    initialize_nvml();
